...
class SelectableBoxLayout_Popup(RecycleDataViewBehavior, BoxLayout):
    ...
    def refresh_view_attrs(self, rv, index, data):
        ...
        return super(SelectableBoxLayout_Popup, self).refresh_view_attrs(
            rv, index, data)
...
class Produto_Nome_Popup(Popup):
    def __init__(self, input_nome,  **kwargs):
        super(Produto_Nome_Popup, self).__init__(**kwargs)
        self.input_nome = input_nome

    def popup_mostrar_artigo_produto(self):
        self.open()
        for nome_produto in produtos_list_event:
            if nome_produto['nome'].lower().find(self.input_nome) >= 0:
                produto = {
                    'codigo': nome_produto['codigo'],
                    'nome': nome_produto['nome'],
                    'preco': nome_produto['preco'],
                    'quantidade': nome_produto['quantidade']
                }
...
class VendasWindow(BoxLayout):
    ...
    def add_produto_nome(self, nome):
        ...
        pesq_nome_pop_up = Produto_Nome_Popup(nome)
        pesq_nome_pop_up.popup_mostrar_artigo_produto()
